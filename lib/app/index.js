	...
	this.lower = function (cb) {
		...
		beforeShutdown(function (err) {
			...
			async.series([
				...
				function teardownadapter(cb) {
					for(var adapter in sails.adapters){
						if(sails.adapters[adapter].teardown !== undefined){
							sails.adapters[adapter].teardown(cb);
						}
					}
				},
